...
def pd_control(
    ...
    # Apply gains.
    x_err *= -kp_kv[:, 0]  # for stiffness
    dx_err *= -kp_kv[:, 1]  # for damping
...
def pd_control_orientation(
    ...
    # Apply gains.
    ori_err *= -kp_kv[:, 0]  # for stiffness
    w_err *= -kp_kv[:, 1]  # for damping
...
def opspace(
    ...
    stiffness: Union[Tuple, np.ndarray] = (200.0, 200.0, 200.0, 200.0, 200.0, 200.0),
    ...
    J_v = np.zeros((3, model.nv), dtype=np.float64)  # linear
    J_w = np.zeros((3, model.nv), dtype=np.float64)  # angular
    mujoco.mj_jacSite(
        model,
        data,
        J_v,
    ...
    # Compute position PD control using position error and velocity error
    ...
    # Compute orientation PD control using orientation error and angular velocity error
    ...
    # Compute generalized forces for the joints based on the PD accelerations
    ...
    # if we want to add gravity compensation
    if gravity_comp:
