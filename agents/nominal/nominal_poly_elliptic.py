...
class NominalPolyElliptic(NominalBase):
    """
    ...
    Other than the NominalPolyRail this controller determines the control point but alters 
    ...
    def get_action(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        current_rotation = R.from_euler("xyz", R.from_quat(curr_quat).as_euler("xyz", degrees=True), degrees=True)
        z_curr = current_rotation.apply(np.array([0, 0, 1]))
        self.indent_rot = self.indent * z_curr
        eef_pos = np.array(obs[28:31]) + self.indent_rot
        # ========================== receive current state from the observation

        # compute next control point (USING ELLIPSOID) ========================
        close_points = OrderedDict()
        overall_dist = []
        for i, point in enumerate(self.control_points):
            # iterate over all control points (POSSIBLE IMPROVEMENT)
            cp_distance = eef_pos - point
            rot_cp_distance = current_rotation.apply(cp_distance, inverse=True)

            overall_dist.append(np.linalg.norm(cp_distance))
            if abs(rot_cp_distance[0]) <= self.action_radius[0]:
                if abs(rot_cp_distance[1]) <= self.action_radius[1]:
                    if abs(rot_cp_distance[2]) <= self.action_radius[2]:
        ...
        # constant gains and computed positional shift
    ...
    def _calculate_orientation(self) -> np.ndarray:
        ...
        orientation_points = slerp(range(0,self.control_points.shape[0])).as_quat()  # the same number as control points
    ...
    def _send_points_to_env(self, point_index: int, num_points: int=5) -> None:
        ...
        Details:
            If the next num_points do not exist in the trajectory, takes the end_directory of the trajectory
            and artificially extend the trajectory following the end_directory.
        ...
        for running_index in range(0, num_points):
            if point_index+running_index < len(self.control_points):
                ...
                current_rotation = R.from_euler("x", R.from_quat(self.orientation_points[point_index]).as_euler("xyz", degrees=True)[-1], degrees=True)
        ...
        corresponding_states = {"positions": np.array(positions), "velocity_dirs": np.array(directions)}
